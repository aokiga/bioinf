...
int source() {
    ...
    int randomPos = ll(fabs(getRandLL())) % n;  // select random point from data
    cerr << randomPos << '\n';
    centers.push_back(pos[randomPos]);
    for (int kk = 1; kk < k; ++kk) {

        vector<ll> dists(n);    // counting distances from data point
        for (int i = 0; i < n; ++i) {
            ld mindist = -1;
            ll closest = -1;
            for (int j = 0; j < kk; ++j) {
                ld curdist = dist2(pos[i], centers[j]);
                if (closest == -1 || mindist > curdist) {
                    closest = j;
                    mindist = curdist;
                }
